...
const goTest = item => {
  ...
  createEpaperScore({epaperId: item.id, level: proxy.$cache.session.getJSON('level')})
    .then(res => {
      ...
      buttonLoading.value = true
      if (res.code === 200) {
        router.push({ path: '/myTest/answer', query: { id: item.id, epaperScore: res.data, name: item.name, qualifiedScore: item.qualifiedScore } })
...
const goContinueTest = item => {
  router.push({ path: '/myTest/answer', query: { id: item.id, epaperScore: item.lastEpaperScoreId, name: item.name, qualifiedScore: item.qualifiedScore, isContinue: true } })
